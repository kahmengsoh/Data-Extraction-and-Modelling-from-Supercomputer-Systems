...
if __name__ == '__main__':
    ...
    for dataKey, dataPath in dataPathDict.items():
        ...
        if csvFilePath is not None:
            ...
            df = df[((df['gtype'] == 'AD') & (df['t1w'] == 1) & (~df['age'].isna()))]
            df['dataKey'] = dataKey
            if dataKey == 'ADHD':
    ...
    cols = ['dataKey', 'ID', 'id', 'sex', 'ses', 'age', 'gtype', 'dx_gtype', 't1w', 'fmriprep', 'freesurf']
    df = df[cols]
    ...
    outFile_selected_ses = 'listxxx.csv'
